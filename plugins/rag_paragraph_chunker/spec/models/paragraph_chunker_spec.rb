...
RSpec.describe RagSteps::ParagraphChunker do
  describe "validations" do
    subject { build(:rag_steps_paragraph_chunker) }
    ...
    describe "overlap_must_be_less_than_size" do
      it "is invalid when overlap >= size" do
        chunker = build(:rag_steps_paragraph_chunker, chunk_size: 200, chunk_overlap: 200)
        expect(chunker).not_to be_valid
      end

      it "is valid when overlap < size" do
        chunker = build(:rag_steps_paragraph_chunker, chunk_size: 1000, chunk_overlap: 200)
        expect(chunker).to be_valid
      end

      it "skips validation when chunk_overlap is blank" do
        chunker = build(:rag_steps_paragraph_chunker, chunk_size: 100, chunk_overlap: nil)
        chunker.valid?
      ...
      it "skips validation when chunk_size is blank" do
        chunker = build(:rag_steps_paragraph_chunker, chunk_size: nil, chunk_overlap: 50)
        chunker.valid?
        expect(chunker.errors[:chunk_overlap]).not_to include("must be less than chunk size")
      end
    end
  ...
  describe ".key" do
    it { expect(described_class.key).to eq("paragraph_chunker") }
  end
  ...
  describe "#chunking_strategy" do
    it { expect(described_class.new.chunking_strategy).to eq("paragraph") }
  end

  describe ".permitted_params" do
    it "returns paragraph_chunker params" do
      params = ActionController::Parameters.new(
        paragraph_chunker: { chunk_size: "800", chunk_overlap: "100", min_paragraph_size: "50" },
      )
      result = described_class.permitted_params(params)
      expect(result[:chunk_size]).to eq("800")
    end
  end
  ...
  describe "#execute" do
    it "delegates to ChunkerExecutor" do
      chunker = build(:rag_steps_paragraph_chunker)
      docs = [Rag::Document.new(id: "1", content: "First paragraph.\n\nSecond paragraph.", metadata: {},
                                chunks: [],)]
  ...
  describe "#validate_configuration!" do
    it "raises when chunk_size is nil" do
      chunker = build(:rag_steps_paragraph_chunker, chunk_size: nil)
    ...
    it "does not raise when chunk_size is valid" do
      chunker = build(:rag_steps_paragraph_chunker, chunk_size: 500)
      expect { chunker.validate_configuration! }.not_to raise_error
    end
  end

  describe ".label" do
    it { expect(described_class.label).to eq("Paragraph") }
  end

  describe ".icon" do
    it { expect(described_class.icon).to eq("fa-solid fa-paragraph") }
  end

  describe ".description" do
    it "returns a non-empty description" do
      expect(described_class.description).to include("paragraph boundaries")
    end
  end
  ...
  describe "#to_configuration" do
    it "returns a serializable hash" do
      chunker = build(:rag_steps_paragraph_chunker, chunk_size: 800, chunk_overlap: 150)
      config = chunker.to_configuration
      expect(config).to include("chunk_size" => 800, "chunk_overlap" => 150)
    end
  end
  ...
  describe "#summary" do
    it "includes chunk_size and chunk_overlap" do
      chunker = build(:rag_steps_paragraph_chunker, chunk_size: 800, chunk_overlap: 150)
      expect(chunker.summary).to eq("Paragraph — 800 chars, 150 overlap")
    end
  end
end
