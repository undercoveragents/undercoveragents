...
class MissionTriggerSchedulerJob < ApplicationJob
  queue_as :default
  ...
  def perform
    loop do
      due_ids = MissionTrigger.due_schedule.order(:next_run_at, :id).limit(BATCH_SIZE).pluck(:id)
  ...
  def dispatch_due_trigger(trigger_id)
    ...
    MissionTriggers::Dispatcher.new(
      mission_trigger: trigger,
  ...
  def claim_due_trigger(trigger_id)
    ...
    MissionTrigger.transaction do
      trigger = MissionTrigger.lock("FOR UPDATE SKIP LOCKED").find_by(id: trigger_id)
  ...
  def record_dispatch_error(trigger_id, trigger, error)
    ...
    Rails.logger.error(
      "[MissionTriggerSchedulerJob] Failed to dispatch trigger #{trigger_id}: #{error.class} - #{error.message}",
    )
  end
end
