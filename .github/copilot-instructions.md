...
- Agent structured-output contracts live on `Agent.configuration` through `response_format` (`text`, `json_object`, `json_schema`) and `response_schema`; apply them through `Llm::ResponseFormat` / `Llm::ChatOptions` so direct agent chats and model-routing fallback/comparison paths stay aligned. Keep schema parsing helpers in `Llm::JsonResponseParser` and expose parsed assistant JSON through `Message#parsed_json_content` without mutating stored messages.
- Agent prompt previews are read-only and deterministic through `Agents::PromptPreview` plus `admin/agents/:id/prompt_preview`; previews must not create `Chat` records, enqueue jobs, execute tools, or contact providers.
- LLM observability uses metadata-only ActiveSupport notification events: `llm.generation.undercover_agents`, `llm.generation_error.undercover_agents`, `llm.tool_call.undercover_agents`, and `llm.stream_chunk.undercover_agents`. Do not add prompt text, response bodies, tool arguments, attachments, credentials, or raw provider payloads to those payloads or to `UndercoverAgents::LlmLogSubscriber`; keep body dumps limited to the disabled `RubyLlmDebugLogging` hook.
