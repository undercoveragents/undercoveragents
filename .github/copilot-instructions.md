...
- Operation-owned agent and tool show pages, plus the mission details/edit page, should expose one-click Duplicate page-hero actions when the record supports duplication; copied missions keep the flow definition but reset undo/redo history, and disabled duplicate actions should use the same policy tooltip behavior as other write controls.
