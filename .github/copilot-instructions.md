...
- Keep the checked-in `pnpm-workspace.yaml` `allowBuilds.esbuild: true` entry so pnpm 11 installs can run the required esbuild postinstall without an interactive `pnpm approve-builds` step in local setup or CI.
