...
- Built-in runtime tools are not user-configurable; define their hardcoded widget presentation alongside their `BuiltinTools::Registry.register` entry.
...
- **Shared LLM chat options:** Agent forms and mission `llm` nodes share `Llm::ChatOptions` for temperature, reasoning/thinking, and provider-specific params. Gate temperature off `Model.capabilities` `temperature`, gate thinking off `reasoning`, and keep provider overrides in validated `custom_llm_params` JSON rather than ad hoc per-surface keys. For DeepSeek, normalize SDK-style `extra_body` custom params into the actual request payload, treat `custom_llm_params.thinking.type = "disabled"` as authoritative over UI thinking effort, and automatically send `thinking.type = "disabled"` when the UI reasoning setting is off or when tools are present so the provider does not silently stay in its default thinking-on mode.
- **DeepSeek reasoning + tools guardrail:** until upstream OpenAI-compatible reasoning round-trips are reliable for tool continuations, `Llm::ChatOptions` must skip DeepSeek thinking mode whenever the chat has tools attached and must send the explicit DeepSeek disable toggle there instead of relying on omitted reasoning params. Keep that compatibility rule centralized in the shared chat-options layer instead of scattering provider checks through jobs or controllers.
...
- Hardcoded runtime-only tools are registered via `BuiltinTools::Registry` and surfaced read-only in Headquarter.
...
Channels are the new tenant-owned invocation/exposure layer for agents and missions. A channel owns routing, enabled/default state, presentation/configuration, external identities, external conversation mapping, channel-specific credentials, and target assignment. Connectors remain credentials/connectivity only, and capabilities remain agent behavior/extensions only.

- **Models:** `Channel` (`channel_type` + `configuration` JSONB, optional `connector_id`, optional `default`, FriendlyId slug, logo attachment), `ChannelTarget` (polymorphic `Agent`/`Mission` target assignment), `ChannelIdentity`, `ChannelConversation`, and `ChannelCredential`.
...
- **Target rule:** `ChannelTarget` must validate target kind support from the configurator and enforce same-tenant targets through the target operation's tenant. Do not expose operation-owned records through tenant-global or foreign-tenant channel assignments.
...
- **Attributes:** `name` (unique per tenant), `channel_type = "client"`, `configuration` (JSONB-backed chat copy/settings), `default` (boolean), default agent target, and `logo` (Active Storage attachment).
...
- **Caching:** `Channel.current_client_channel` and `Channel.current_client_settings` cache the default client-type channel payload used by the public chat layout and preview surfaces.
- **Default flag:** keep one default client-type channel per tenant so public `/chat` resolution has a single primary branding source.
...
- **Model:** `Tenant` — `name` (unique), FriendlyId slug, `description`, `has_many :operations`, `has_many :users`, `has_many :connectors`, `has_many :channels`, `has_one :system_preference`.
...
- **Global tenant-owned models:** `User`, `Operation`, `Connector`, `Channel`, and `SystemPreference` carry a non-null `tenant_id`. Keep cross-tenant validations on associations such as channel target assignment and system-preference connectors.
- **Tenant-scoped helpers:** use `scoped_operations`, `scoped_connectors`, `scoped_channels`, `tenant_scoped_test_suites`, `tenant_scoped_mission_runs`, and `tenant_scoped_chats` from `ApplicationController` instead of global queries.
...
- It edits agent records through the generic `manage_record` and `navigate_to_page` builtins with `resource="agent"`, using `assigned_tool_ids` for existing tools and `subagent_ids` for existing subagents. Treat `navigate_to_page` as a post-change UI handoff only; it does not return page content or agent data.
...
- Turning off thinking/reasoning on an agent means storing `thinking_effort = "none"` and clearing `thinking_budget`, not setting reasoning fields to `null`; runtime agent updates normalize blank DeepSeek reasoning updates to that explicit off state so the provider does not fall back to default thinking.
...
- It edits tool records through the generic `manage_record` and `navigate_to_page` builtins with `resource="tool"`, using nested `toolable_attributes` for type-specific config. Treat `navigate_to_page` as a post-change UI handoff only; it does not return page content or tool data.
...
- **CRUD + navigation tools:** Mission Designer can use the generic builtin `manage_record` and `navigate_to_page` tools, both backed by `RuntimeRecords::Registry`. Keep those tools resource-agnostic, register supported resources there, scope operation-owned records through the current operation and tenant, and treat `navigate_to_page` as a final UI handoff only after update work or after creates that did not already navigate; it does not read page content or return mission data. When the agent must create and fully author a new mission in one turn, let `manage_record` navigate to the designer immediately, then target that returned mission through the mission-designer tools' explicit `mission_id` support. After a standalone `navigate_to_page` moves to a different mission page, do not continue flow edits in the same turn; mission-create navigation may continue only through the returned `mission_id`.
