...
| Misc | amoeba (deep cloning), Chartkick + Groupdate, fugit, rubyzip, stream-markdown-parser, pnpm |
...
- **Automation triggers:** missions attach shared `AutomationTrigger` records through `Mission#automation_triggers` / `Mission#mission_triggers`, exposed under `/admin/missions/:mission_id/automation`. The automation layer is operation-owned and polymorphic so the same schedule/webhook model also works for other resources such as `RagFlow`. Keep mission compatibility wrappers only for legacy call sites and specs; new code should target `AutomationTrigger`.
- **Designer:** React Flow canvas (`app/javascript/mission_designer/`). Flow saved via `PATCH save_flow`. Stimulus: `mission` controller.
- **Scheduled dispatch:** `AutomationTriggerSchedulerJob` is registered from `config/recurring.yml` every minute, claims due schedule triggers with DB locking, advances `next_run_at` before dispatch, and hands off execution to `AutomationTriggers::Dispatcher`, which fans out to the target-specific runtime path (Mission -> mission run job, RAG flow -> RAG execution job). Public webhook POSTs go through `/api/v1/automation_webhooks/:id` with the `X-UndercoverAgents-Webhook-Secret` header, while `/api/v1/mission_webhooks/:id` remains a compatibility alias.
...
- **Automation triggers:** `RagFlow` also attaches shared `AutomationTrigger` records under `/admin/rag_flows/:rag_flow_id/automation`, so scheduled and webhook-driven indexing runs use the same generic automation layer as missions.
